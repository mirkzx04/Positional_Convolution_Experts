...
This project was created to study the training dynamics of a Vision Mixture-of-Experts (MoE) architecture in a context different from classic Transformer architectures.
...
The model was trained on TinyImageNet, consisting of 200 classes and images resized to 224 x 224. The main contribution of the project is the analysis of routing dynamics. In particular, two aspects are distinguished:
...
### Architecture 
<img src="img/PCE_architecture.png" width="1000" align="left" style="margin-right: 20px; margin-bottom: 10px;">

**MoE Blocks** 
The MoE blocks operate at the patch level on the feature map. Each input feature map is divided by the `Patch Extractor` into patches of a specific size (e.g., P_s x P_s). For each patch, positional features are then calculated via `Fourier Positional Features`, which are concatenated to the patch content and used exclusively by the router.
...
To integrate the experts' work, a **first residual connection** is applied. The output at this stage is the sum of the original input patches and the aggregated expert output (the latter being scaled by a learnable parameter, gamma, initialized to a small value). This residual has two main functions: preserving the original signal during the early stages of training and making the experts' contribution gradual, preventing sparse routing from destabilizing the representation too early.

At this point, the processed patches are recomposed into their original spatial arrangement via a **rearrange** operation, forming a standard multi-dimensional tensor (Batch, Channels, Height, Width).

The reconstructed feature map is then normalized and activated. It passes through **Group Normalization (GN)** followed by a **SiLU** activation function. This step serves to stabilize the feature distribution after sparse processing and before the subsequent dense phase.

The third block is a **shared convolutional block**, consisting of a 3x3 Convolution, followed by Group Normalization and SiLU. Its role is to locally mix the features produced by the experts. This is important because patches are processed separately and then reinserted in their original position: without a local dense operation, discontinuities could emerge between adjacent patches or overly sharp boundaries between regions processed by different experts.

Finally, the layer applies **two final residual connections**. First, it adds the output of the dense convolutional block to the normalized MoE output. Then, it adds this combined result back to the reconstructed feature map. In this way, the block combines three components: sparse routing on the experts, shared normalization/activation, and dense convolutional post-processing.

**Router Gate** 
The `Router Gate` is the module that assigns each patch to one of the available experts. For each patch, the gate builds a compact representation by concatenating the **spatial mean** and the **spatial maximum** of the patch. In this way, each patch is represented by a vector summarizing its average and peak activations.

This representation is Layer Normalized and passed to a linear layer (multiplied by weights and biased) to produce a specific **logit score for each expert**. The `Router Gate` can therefore be seen as a function that, given the representation of a patch, assigns a score to each expert in the pool.

The logits are then scaled by a **temperature parameter** and transformed into probabilities via a **softmax function**. The temperature controls how uniform or selective the distribution over the experts is: higher values produce softer distributions, while lower values make the logits sharper and push the router towards more definitive choices.
...
However, each expert has a maximum capacity, which limits the number of patches it can process in a single forward pass. The **capacity limit** is calculated by multiplying the total number of patches by a capacity factor, dividing by the number of experts, and rounding up to the nearest integer. If an expert receives more patches than its capacity, only the patches with the highest routing probability are kept, while the others are dropped from the sparse path.
...
### Training 
The model was trained on TinyImageNet with 224 x 224 images. The following setup was used for all training runs:
...
The router is also equipped with additional hyperparameters: **Tau** serves to regulate the temperature in the logits, and **Aux Loss Weight** regulates the weight of the load balancing loss. 

The weight of the auxiliary loss follows a specific schedule throughout training:
*   It starts at zero for the initial epochs.
*   Once the router's active phase begins, it scales linearly to a peak value during the warmup period.
*   It maintains this peak value for a set number of epochs.
*   Finally, it decays smoothly toward a final, lower value using a cosine function.
...
2. **Dense Post-Processing**: The second hypothesis concerns the role of dense blocks after the sparse compute. An overly expressive dense post-processing could absorb part of the experts' capacity and become the main driver of backpropagation. The test was conducted by replacing the 3 x 3 kernel of the post-block after the `rearrange` with a 1 x 1 kernel. This modification also did not eliminate overfitting, but it showed that the dense block is important for performance, given that reducing it worsens the Top-1 accuracy.
